...
#include <utility>
#include "br2_fsm_bumpgo_cpp/BumpGoNodeEx2OL.hpp"
...
namespace br2_fsm_bumpgo_cpp_ex2_ol
{
...
BumpGoNodeEx2OL::BumpGoNodeEx2OL()
  ...
  state_(FORWARD), rotation_duration(0)
{
  ...
  scan_sub_ = create_subscription<sensor_msgs::msg::LaserScan>(
    ...
    std::bind(&BumpGoNodeEx2OL::scan_callback, this, _1));  // scan_callback will be called whenever a new message is received on the topic. The std::bind() function is used to bind the callback function to the BumpGoNode class instance (this) and pass the received message as an argument (_1).

  vel_pub_ = create_publisher<geometry_msgs::msg::Twist>("output_vel", 10);  // default QoS is used (which is reliable + volatile)
  timer_ = create_wall_timer(50ms, std::bind(&BumpGoNodeEx2OL::control_cycle, this));

  state_ts_ = now();  // now() gets the current time from rclcpp::Clock
}

void
BumpGoNodeEx2OL::scan_callback(sensor_msgs::msg::LaserScan::UniquePtr msg)
{
...
BumpGoNodeEx2OL::control_cycle()
{
  ...
  switch (state_) {
    case FORWARD:
      ...
      // added code
      if ( check_forward_2_back() || check_forward_2_turn_right() || check_forward_2_turn_left()){

        best_angle_index = find_farthest_obstact_angle_index();
        rotation_duration = calculate_rotation_duration();
        
        if (best_angle_index <= last_scan_->ranges.size()/2){
          go_state(TURN_RIGHT);
          RCLCPP_INFO(this->get_logger(), "Turning Right for %f", rotation_duration);
        }
        else{
          go_state(TURN);
          RCLCPP_INFO(this->get_logger(), "Turning Left for %f", rotation_duration);
        }
      }
...
void
BumpGoNodeEx2OL::go_state(int new_state)
{
  state_ = new_state;
  state_ts_ = now();
}
...
bool
BumpGoNodeEx2OL::check_forward_2_back()
{
...
bool
BumpGoNodeEx2OL::check_forward_2_stop()
{
...
bool
BumpGoNodeEx2OL::check_stop_2_forward()
{
...
bool
BumpGoNodeEx2OL::check_back_2_turn()
{
...
bool
BumpGoNodeEx2OL::check_turn_2_forward()
{
  // added code
...
// added code
bool
BumpGoNodeEx2OL::check_forward_2_turn_left()
{
  // checking if there is an obstacle on the right, and turning left if this is the case
  size_t right_diag = last_scan_->ranges.size() * (4.0 / 10.0);
...
bool
BumpGoNodeEx2OL::check_forward_2_turn_right()
{
  // checking if there is an obstacle on the left, and turning right if this is the case
  size_t left_diag = last_scan_->ranges.size() * (6.0 / 10.0);
...
size_t 
BumpGoNodeEx2OL::find_farthest_obstact_angle_index()
{
...
rclcpp::Duration
BumpGoNodeEx2OL::calculate_rotation_duration()
{
  if (best_angle_index <= last_scan_->ranges.size() / 2){
